...
function Server(conf) {
  ...
  this.on('listening', function() {
    self.log('Listening on unix socket \x1b[1m%s\x1b[m.', process.env.HOME + "/.tty.js/sock");
  });
...
Server.prototype.listen = function(path, func) {
  return this.server.listen(path || this.conf.sockpath || "/var/run/tty.js.sock", func);
};
